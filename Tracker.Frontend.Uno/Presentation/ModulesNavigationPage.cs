...
public sealed partial class ModulesNavigationPage : Page
{
    ...
    {
        var logic = new ModulesNavigationPageLogic();
        var ui = new ModulesNavigationPageUi(logic);

        DataContext ??= (Application.Current as App).Host.Services.GetRequiredService<ModulesNavigationViewModel>();

        this.NavigationCacheMode(NavigationCacheMode.Required).Background(Theme.Brushes.Background.Default)
            .Content(ui.BuildContent((ModulesNavigationViewModel) DataContext));
    }

    private class ModulesNavigationPageUi
    {
        private readonly ModulesNavigationPageLogic logic;

        internal ModulesNavigationPageUi(ModulesNavigationPageLogic logic)
        {
            this.logic = logic;
        }

        internal Grid BuildContent(ModulesNavigationViewModel viewModel)
        {
            var grid = new Grid();
        ...
        private ListView BuildNavigationListView(ModulesNavigationViewModel viewModel)
        {
            ...
            var options = logic.GetModules().Select(module =>
            {
        ...
        private Grid BuildContentGrid(ModulesNavigationViewModel viewModel)
        {
            ...
            foreach (TrackerModule module in logic.GetModules())
            {
            ...
            logic.SetContentGridsDictionary(contentGrids);

            return grid;
        }
    }

    private class ModulesNavigationPageLogic
    {
        ...
        internal void SetContentGridsDictionary(Dictionary<TrackerModule.Module, Grid> contentGrids)
        {
        ...
        internal List<TrackerModule> GetModules()
        {
            return modules;
        }
    }
